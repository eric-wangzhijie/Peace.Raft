...
namespace Uni.Core.RPC
{
    ...
    public abstract class AbstractServer<TProtocol> : IServer
    {
        ...
        public AbstractServer(string id, string host, int port, string clusterToken, ILog logger, string redisConnection)
        {
        ...
        public async Task Start()
        {
            ...
            if (serviceNames != null)
            {
                ...
                {
                    await _registry.Register(new ServiceDescription(_id, serviceName, _serverStub.Host, _serverStub.Port));
                }
                _serverStub.Functions.Add(uint.MaxValue, Renew);
            }
            else
            {
        ...
        public async Task Stop()
        {
            ...
            if (serviceNames != null)
            {
                foreach (string serviceName in serviceNames)
                {
                    await _registry.Deregister(new ServiceDescription(_id, serviceName, _serverStub.Host, _serverStub.Port));
                }
            }
        }
            ...
            if (DateTime.Now.Subtract(_renewTime).TotalSeconds > RenewIntervalSeconds)
            {
                ...
                if (serviceNames != null)
                {
                    foreach (string serviceName in serviceNames)
                    {
                        try
                        {
                            await _registry.Renew(new ServiceDescription(_id, serviceName, _serverStub.Host, _serverStub.Port));
                        }
                        catch (Exception ex)
                        {
